	...
	if (serial.state == MUX_STATE_MUXING)
		for (i=1;i<GSM0710_MAX_CHANNELS;i++)
			if (channellist[i].fd < 0) // is this channel free?
			{
				...
				snprintf(linkname, linksize, "/dev/phone_at%d", i);
				symlink(pts, linkname);
				// Fire property
				const int size = sizeof("gsm0710mux.channelz");
				char property[size];
				snprintf(property, size, "gsm0710mux.channel%d", i);
	...
	int to)
{
	...
	LOGMUX(LOG_INFO, "Wrote %d bytes", wrote);
...
SYSCHECK(tcdrain(serial_device_fd));
	...
	do
	{
		...
		if (FD_ISSET(serial_device_fd, &rfds))
		{
			...
			{
				LOGMUX(LOG_INFO, "Received OK");
				return 0;
			}
			if (memstr((char *) buf, len, "ERROR"))
			{
				LOGMUX(LOG_INFO, "Received ERROR");
				return -1;
			}
		}
	} while (sel);
...
*/
    //KB - we set CMUX mode basic unconditionally
